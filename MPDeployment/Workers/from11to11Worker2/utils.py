...
def get_model(model_name, shape, name="data"):
    """
    Loads an MXNet model symbol without binding it yet.
    Binding will be done at runtime with the correct input shape.
    """
    model_path = f"{model_name}.json"
    if not os.path.exists(model_path):
        raise FileNotFoundError(f"Model file {model_path} not found!")

    sym = mx.symbol.load(model_path)
    mod = mx.mod.Module(symbol=sym, data_names=[name], label_names=None)
    mod._is_bound = False  # custom flag to track if model has been bound yet
    return mod
...
def execute_model(event, mod):
    try:
        ...
        input_ndarray = mx.nd.array(input_array)

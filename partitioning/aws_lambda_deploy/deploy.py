...
for name in os.listdir(models_path):
    if 'json' in name:
        ...
        if not int(func_id) == 0:
            ...
            # mxnet needs libgomp.so.1 at runtime; py3.10/AL2 base image does NOT ship it,
            # so every worker (like master) must attach the libgomp layer.
            body_list.append('      Layers:\n')
            body_list.append('        - arn:aws:lambda:ca-central-1:<YOUR_ACCOUNT_ID>:layer:libgomp-layer:2\n')
            body_list.append('\n')

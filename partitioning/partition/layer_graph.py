...
# from parallel_util import hyper_params


# === ROI: local copies of HB types for Gillis =================================
...
class Instance:
    def __init__(self, cpu: float, mem: Union[float, None], gpu: Union[int, None]) -> None:
        self.cpu = cpu
        self.mem = self.cpu * 2 if mem is None else mem
        self.gpu = gpu
...
class FunctionCost:
    ...
    def calculate_total_cost_with_workers(self, master_latency: float,
                                          ...
                                          worker_instances: List[Instance],
                                          billed_second: bool = True) -> float:
        master_cost = self.cost(master_latency/1000, batch_size, master_instance, billed_second)
        worker_total_cost = 0.0
        for idx, wl in enumerate(worker_latencies):
            worker_total_cost += self.cost(wl/1000, batch_size, worker_instances[idx], billed_second)
        return master_cost + worker_total_cost
...
def load_onnx_model(model_path, with_external_data = False):
    if os.path.isdir(model_path): # multiple sub models
        model_pathes = glob.glob(f'{model_path}/*.onnx')
        model_pathes.sort()
        #-----
        print('umad', model_pathes[0], with_external_data)
        base_model = onnx.load(model_pathes[0], load_external_data=with_external_data)
        #----
        ...
        # print('umad3', model_pathes[0], with_external_data)
        model = onnx.load(model_path, load_external_data=with_external_data)
...
class MeanPredictor():
        # --- ROI: expose per-stage function durations (ms) -----------------------
    def stage_function_durations_ms(self, stage):
        """
        Returns (master_ms, worker_ms_list) for this stage.
        Master time includes encode/await/comm/decode per Gillis.
        Worker time is each worker's own de + compute + en.
        """
        worker_models = [m for m, fid in zip(stage.models, stage.func_ids) if fid != 0]
        if len(worker_models) == 0:
            # master-only stage
            return model_latency(stage.models[0]), []

        # Workers: individual local runtimes
        worker_ms = [
            cal_de_overhead(m.get_in_shape()) + model_latency(m) + cal_en_overhead(m.get_out_shape())
            for m in worker_models
        ]

        # Master: encode all inputs + wait (max worker + comm) + decode all outputs
        enc_sum = sum([cal_en_overhead(m.get_in_shape()) for m in worker_models])
        dec_sum = sum([cal_de_overhead(m.get_out_shape()) for m in worker_models])
        # Wait time per Gillis = max(worker local time) + comm
        comm_time = cal_comm_overhead(len(worker_models), *worker_models[0].get_in_out_shape())
        wait_time = (max(worker_ms) if worker_ms else 0.0) + comm_time
        master_ms = enc_sum + wait_time + dec_sum
        return master_ms, worker_ms

    def plan_function_durations_ms(self, plan):
        master_ms_total = 0.0
        worker_ms_all = []
        for s in plan.stage_list:
            m_ms, w_ms = self.stage_function_durations_ms(s)
            master_ms_total += m_ms
            worker_ms_all.extend(w_ms)
        return master_ms_total, worker_ms_all
    
    #------------------------
    def debug_plan(self, plan):
        from parallel_util import hyper_params
        mem_mb = hyper_params.get('func_limit', None)
        print(f"[DEBUG] func_limit (MB) = {mem_mb}")
        tot_lat = 0.0
        for idx, s in enumerate(plan.stage_list, 1):
            st_lat = self._stage_latency(s)
            m_ms, w_ms = self.stage_function_durations_ms(s)
            print(f"[DEBUG] Stage {idx}: shape={s.partition_shape}, workers={len(w_ms)}, stage_lat={st_lat:.3f} ms")
            print(f"        master_ms={m_ms:.3f} ms, worker_ms={['%.3f' % x for x in w_ms]}")
            tot_lat += st_lat
        print(f"[DEBUG] plan_lat (sum stages) = {tot_lat:.3f} ms ; plan.cal_latency() = {self.plan_latency(plan):.3f} ms")

        # cost breakdown
        from parallel_util import vcpu_for_mem
        vcpu = vcpu_for_mem(mem_mb)
        mem_gb = float(mem_mb)/1024.0 if mem_mb is not None else None
        print(f"[DEBUG] billing config: vCPU={vcpu}, mem_gb={mem_gb}")
        master_ms, worker_ms = self.plan_function_durations_ms(plan)
        master_inst = Instance(cpu=vcpu, mem=mem_gb, gpu=0)
        worker_insts = [Instance(cpu=vcpu, mem=mem_gb, gpu=0) for _ in worker_ms]
        master_cost = COST.cost(master_ms/1000.0, 1, master_inst)
        worker_costs = [COST.cost(w/1000.0, 1, inst) for w, inst in zip(worker_ms, worker_insts)]
        print(f"[DEBUG] master_cost={master_cost:.10f}, worker_costs={[float(f'{c:.10f}') for c in worker_costs]}")
        print(f"[DEBUG] total_cost={master_cost+sum(worker_costs):.10f} ; plan.cal_cost() = {self.plan_cost(plan):.10f}")
    # -------------------------------------------------------------------------
    ...
    def plan_cost(self, plan):
        """
        ROI: total Alibaba cost = cost(master) + sum(cost(workers)).
        Instance configs are derived from Gillis' memory limit and your vCPU mapping.
        """
        # Import helpers without circular dependencies
        from parallel_util import vcpu_for_mem
        # Memory key for this plan (Gillis uses memory tiers)
        mem_key = hyper_params.get('func_limit', 3008)  # whatever you're optimizing over

        # Build instances: vCPU from mapping, memory in **GB**
        vcpu = vcpu_for_mem(mem_key)
        mem_gb = int(mem_key) / 1000  # NOTE: assumes mem_key is in MB; adjust if you use GB already

        master_inst = Instance(cpu=vcpu, mem=mem_gb, gpu=0)
        # We assume all workers at same config; override here if you want per-worker configs
        master_ms, worker_ms = self.plan_function_durations_ms(plan)
        worker_insts = [Instance(cpu=vcpu, mem=mem_gb, gpu=0) for _ in worker_ms]
        # print("master ms:", master_ms)
        # print("worker ms:", worker_ms)
        # print('worker instances:', worker_insts)
        
        total_cost = COST.calculate_total_cost_with_workers(
            master_latency=master_ms,
            worker_latencies=worker_ms,
            batch_size=1,
            master_instance=master_inst,
            worker_instances=worker_insts,
            billed_second=True
        )
        return total_cost

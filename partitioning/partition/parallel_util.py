...
num_choice = [1, 2, 4]
# num_choice = [1, 2, 4, 8]
MAX_WORKER_PER_STAGE = 3
MAX_STAGES = 2

# === ROI: single-function baselines & mapping =================================
# Use "memory" for Gillis (AWS-style) or "vcpu" for Alibaba-style numbers.
BASELINE_MODE = "memory"     # or "vcpu"
_SINGLE_LATENCY = {}         # {key -> ms}
_SINGLE_COST = {}            # {key -> currency}
_MEM_TO_VCPU = {}            # {memory_key -> vcpu_value}  # needed if BASELINE_MODE="memory" but your baselines are by vCPU

def set_baseline(mode, latency_map, cost_map=None, mem_to_vcpu=None):
    """ROI: register single-function baselines for ROI computation."""
    global BASELINE_MODE, _SINGLE_LATENCY, _SINGLE_COST, _MEM_TO_VCPU
    BASELINE_MODE = mode
    _SINGLE_LATENCY = dict(latency_map or {})
    _SINGLE_COST = dict(cost_map or {})
    _MEM_TO_VCPU = dict(mem_to_vcpu or {})

def _nearest_key(d, key):
    if not d: return None
    if key in d: return key
    try:
        ks = sorted(d.keys(), key=float)
        return min(ks, key=lambda k: abs(float(k) - float(key)))
    except Exception:
        return key if key in d else None

def single_latency_for(key):
    """ROI: baseline latency lookup by memory or vCPU."""
    if BASELINE_MODE == "memory" and key not in _SINGLE_LATENCY and _MEM_TO_VCPU:
        v = _MEM_TO_VCPU.get(key)
        if v is not None:
            key = _nearest_key(_SINGLE_LATENCY, v)
            return _SINGLE_LATENCY.get(key)
    key = _nearest_key(_SINGLE_LATENCY, key)
    return _SINGLE_LATENCY.get(key)

def single_cost_for(key):
    """ROI: baseline cost lookup by memory or vCPU."""
    if not _SINGLE_COST:
        return None
    if BASELINE_MODE == "memory" and key not in _SINGLE_COST and _MEM_TO_VCPU:
        v = _MEM_TO_VCPU.get(key)
        if v is not None:
            key = _nearest_key(_SINGLE_COST, v)
            return _SINGLE_COST.get(key)
    key = _nearest_key(_SINGLE_COST, key)
    return _SINGLE_COST.get(key)

def vcpu_for_mem(mem_key):
    """
    ROI: vCPU to use for cost of a function running at 'mem_key'.
    Priority: hyper_params['vcpu'] -> _MEM_TO_VCPU mapping -> error.
    """
    v = None
    if 'vcpu' in hyper_params and hyper_params['vcpu'] is not None:
        v = hyper_params['vcpu']
    elif _MEM_TO_VCPU:
        mk = _nearest_key(_MEM_TO_VCPU, mem_key)
        v = _MEM_TO_VCPU.get(mk)
    if v is None:
        raise ValueError("vcpu_for_mem(): No vCPU mapping. Set hyper_params['vcpu'] "
                         "or pass mem_to_vcpu=... to set_baseline().")
    return v
# ==============================================================================
...
def param_partition(layer, parallel_num, main_func_limit):
    if parallel_num > MAX_WORKER_PER_STAGE:
        return None
    models = []
    locs = []
...
def attr_partition(graph, layers, shape, main_func_limit):
    num_workers = shape[0] * shape[1]
    if num_workers > MAX_WORKER_PER_STAGE:
        return None
    func_limit = hyper_params['func_limit']

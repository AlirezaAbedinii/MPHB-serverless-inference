...
# === ROI: optimizer mode ======================================================
OPTIMIZE_BY_ROI = True
ROI_EPS = 1e-9

from parallel_util import single_latency_for, single_cost_for

def _baseline_key_for_optimizer():
    # For Gillis, treat hyper_params['func_limit'] as the memory key
    return hyper_params.get('func_limit', None)

def _plan_roi(plan, baseline_key):
    """
    ROI = (single_latency / plan_latency) / (plan_cost / single_cost)
    Returns (roi, plan_latency, plan_cost). Raises if baseline missing.
    """
    plan_lat = plan.cal_latency()
    plan_cost = plan.cal_cost()

    base_lat = single_latency_for(baseline_key)
    if base_lat is None:
        raise ValueError(f"No single-function baseline latency for key={baseline_key}")
    base_cost = single_cost_for(baseline_key)

    speedup = base_lat / plan_lat if plan_lat > 0 else float('inf')
    if base_cost is None:
        roi = speedup  # falls back to speedup-only if you didn’t register cost
    else:
        cost_loss = (plan_cost / base_cost) if base_cost > 0 else float('inf')
        roi = speedup / cost_loss
    return roi, plan_lat, plan_cost
# =============================================================================
    ...
    def gen_opt_plan_recursive(stages, main_func_limit):
        ...
        if required_mem <= main_func_limit:
            ...
            for main_size in range(0, main_func_limit + 1, step):
                ...
                plan = ExecutionPlan(exe_stages)
                if OPTIMIZE_BY_ROI:
                    baseline_key = _baseline_key_for_optimizer()
                    test_roi, test_lat, test_cost = _plan_roi(plan, baseline_key)
                    if ('opt_roi' not in locals()):
                        opt_roi = float('-inf')
                    if (test_roi > opt_roi + ROI_EPS) or \
                    (abs(test_roi - opt_roi) <= ROI_EPS and ((test_lat < opt_lat) or
                                                            (abs(test_lat - opt_lat) < lat_interval and test_cost < opt_cost))):
                        opt_stages = exe_stages
                        opt_lat = test_lat
                        opt_cost = test_cost
                        opt_roi = test_roi
...
def gen_plan(info_graph):
    ...
    if OPTIMIZE_BY_ROI:
        try:
            r, _, _ = _plan_roi(plan, _baseline_key_for_optimizer())
            logging.info(f'plan ROI: {r}')
        except Exception as e:
            logging.warning(f'ROI not computed: {e}')

...
if __name__ == "__main__":
    ...
    default_partition_cfg = Cfg(harmony.Instance(0.5, 1, None), 1, 1, 1, 1, 1, function_type='partitioned')
    ...
    idle_cpu_functions : List[Union[csreqest.Function, None]] = [csreqest.Function("WRN50-5-single", cpu_url, default_cpu_cfg, config) for _ in range(app_num)]
    gpu_functions : List[Union[csreqest.Function, None]] = []
    idle_gpu_functions : List[Union[csreqest.Function, None]] = [csreqest.Function("gwrn50-5_single", gpu_url, default_gpu_cfg, config) for _ in range(app_num)]
    ...
    for iter in range(duration_min):
        ...
        for function in cpu_functions:
            assert function is not None
            print("batch size is", function.cfg.batch_size)
            function.start(1)

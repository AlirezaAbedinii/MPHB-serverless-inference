...
class HarmonyGroup():
    def __init__(self, apps: List[App], function_type = ["CPU",  "GPU", "partitioned"], ratio : float = 1) -> None:
        self.apps = apps
...
def Harmony(apps : List[App]):
    ...
    while True:
        ...
        if end_id == 0:
            ...
            # only one CPU group → nothing to merge
            start_id = end_id
            continue
        else:
            # collect all apps in CPU segment [start_id, end_id)
            merged_apps = Apps()
            for i in range(start_id, end_id):
                merged_apps.extend(groups[i].apps)

            # try merging: provision merged CPU group
            merged_group = HarmonyGroup(merged_apps.apps, function_type=["CPU","GPU"],
                                        ratio=sum(g.ratio for g in groups[start_id:end_id]))

            # compute cost before merge
            cost_before = sum(g.group_cost() for g in groups[start_id:end_id])
            cost_after = merged_group.group_cost()

            if cost_after < cost_before:
                # ---- CRITICAL FIX ----
                # replace the ENTIRE region [start_id:end_id) with ONLY the merged group
                groups = groups[:start_id] + [merged_group] + groups[end_id:]
                cost_change.append(total_cost(groups))
                stage.append(1)
                start_id = start_id  # continue merging from here
            else:
                start_id = end_id
    ...
    while True:
        ...
        if end_id == 0:
            ...
            # else:
            #     start_id = start_id + 1
            # ---- FIXED GPU MERGING BLOCK ----
            merged_apps = Apps([])
            for i in range(start_id, end_id):
                merged_apps.add(groups[i].apps)

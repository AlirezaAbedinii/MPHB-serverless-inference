...
class Mem:
    ...
    def get_gpu_mem(self, gpu: float, batch: Union[int, None] = None):
        ...
        if mem % 64 != 0:
            mem = ((mem // 64) + 1) * 64
        cpu_min = gpu / 2  # Tesla 16 GB instance: 16 GB GPU → 8 vCPU (ratio 2:1, not 3:1)
        mem = mem / 1024
        if mem > cpu_min * 4:
            return None
        return mem
...
class Cfg:
    ...
    def __str__(self):
        ret = "cpu:\t\t%0.2f" % self.instance.cpu + "\n" + \
            ...
            "cost:\t\t%0.3e" % self.cost + "\n" \
            "slo:\t\t" + str(self.slo) + "\n" \
            "function type:\t\t" + str(self.function_type) + "\n" \
    ...
    def partition_or_not(self, cfg: Union["Cfg", None]) -> bool:
        if cfg is None:
            return False
        elif self.cost > cfg.cost:
            return True
        else:
            return False

...
class Application:
    ...
    def get_app(self, iter):
        ...
        return util.App(self.name, self.slo, float(rps) / 600)
...
def generate_requests_helper(apps : List[util.App], batch_size: int, time_out: List[float], inter_arrival: List[float], duration_min: float, config: dict, ins : util.Instance, lat_cal, q : queue.Queue, worker_instances=None):
    ...
    while True:
        ...
        if count < batch_size and total_delay + delay <= current_timeout:
            ...
            t = serverless.Serverless(queues, config["function_url"], ins, lat_cal, que,worker_instances=worker_instances)
            t.start()
            threads.append(t)
            queues = []
            count = 0
...
def generate_requests(cfg : util.Cfg, duration_min: int, config: dict, lat_cal, q : queue.Queue):
    ...
    master_instance = cfg.instance
    worker_instances = None

    ftype = getattr(cfg, "function_type", "single")
    if ftype == "partitioned":
        topo = getattr(cfg, "topology", {}) or {}
        master_topo = topo.get("master", {})
        workers_topo = topo.get("workers", [])

        # Master override from topology (fall back to cfg.instance if missing)
        m_cpu = master_topo.get("cpu", master_instance.cpu)
        m_mem = master_topo.get("mem", master_instance.mem)
        master_instance = util.Instance(m_cpu, m_mem, None)

        # Workers from topology
        worker_instances = []
        for w in workers_topo:
            w_cpu = w.get("cpu", None)
            w_mem = w.get("mem", None)
            if w_cpu is None or w_mem is None:
                continue
            worker_instances.append(util.Instance(w_cpu, w_mem, None))
    
    # return generate_requests_helper(cfg.apps, batch_size, time_out, inter_arrival, duration_min, config, cfg.instance, lat_cal, q)
    return generate_requests_helper(
        cfg.apps,
        batch_size,
        time_out,
        inter_arrival,
        duration_min,
        config,
        master_instance,      # <<< use topology-aware master
        lat_cal,
        q,
        worker_instances      # <<< NEW: pass worker instances
    )
...
class Sample:
    ...
    def create_client(
        ...
        config.endpoint = f'<your-account-id>.us-east-1.fc.aliyuncs.com'
